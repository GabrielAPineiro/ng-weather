...
export class WeatherService {
	...
	addCurrentConditions(zipcode: string): void {
		const cachedConditions = this.cacheService.getCachedItem(zipcode, 'conditions');
		...
		if (!cachedConditions) {
			...
			this.http
				...
				.pipe(tap((data) => this.cacheService.setCachedItem(zipcode, { zip: zipcode, data }, null)))
				.subscribe((data) => this.currentConditions.update((conditions) => [...conditions, { zip: zipcode, data }]));
		} else {
			this.currentConditions.update((conditions) => [...conditions, cachedConditions as ConditionsAndZip]);
		}
	}
	...
	getForecast(zipcode: string): Observable<Forecast> {
		const cachedForecast = this.cacheService.getCachedItem(zipcode, 'forecast');

		if (cachedForecast) {
			return of(cachedForecast as Forecast);
		}
		...
		return this.http
			...
			.pipe(tap((forecast) => this.cacheService.setCachedItem(zipcode, null, forecast)));
	}
